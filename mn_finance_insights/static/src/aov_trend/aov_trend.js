...
export class AOVTrend extends Component {
    static template = "mn_finance_insights.AOVTrend";
    static props = { "*": true };
    setup() {
    ...
    async load() {
        ...
        try { this.state.data = await this.orm.call("finance.aov.trend", "get_aov_data", [this.state.options]); }
    ...
    renderChart() {
        ...
        const c = this.rootRef.el.querySelector("canvas[data-chart='aov']");
        if (!c) return;
        const d = this.state.data;
        this.chart = new window.Chart(c, {
            type: "line",
            data: { labels: d.labels,
                datasets: [{ label: "Average Order Value", data: d.aovs, borderColor: "#6366f1",
                    backgroundColor: "rgba(99,102,241,0.15)", fill: true, tension: 0.35, pointRadius: 4, borderWidth: 3 }] },
...
registry.category("actions").add("mn_finance_insights.aov_trend", AOVTrend);
