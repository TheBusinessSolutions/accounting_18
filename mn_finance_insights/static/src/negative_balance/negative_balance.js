...
export class NegativeBalance extends Component {
    static template = "mn_finance_insights.NegativeBalance";
    static props = { "*": true };
    setup() {
    ...
    async load() {
        ...
        try { this.state.data = await this.orm.call("finance.negative.balance", "get_negative_data", [this.state.options]); }
...
registry.category("actions").add("mn_finance_insights.negative_balance", NegativeBalance);
