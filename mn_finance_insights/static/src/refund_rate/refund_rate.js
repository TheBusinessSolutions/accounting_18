...
export class RefundRate extends Component {
    static template = "mn_finance_insights.RefundRate";
    static props = { "*": true };
    setup() {
    ...
    async load() {
        ...
        try { this.state.data = await this.orm.call("finance.refund.rate", "get_refund_data", [this.state.options]); }
    ...
    renderChart() {
        ...
        const c = this.rootRef.el.querySelector("canvas[data-chart='refund']");
        if (!c) return;
        ...
        this.chart = new window.Chart(c, {
            ...
            data: { labels: d.labels,
                datasets: [
                    { type: "bar", label: "Revenue", data: d.revenue, backgroundColor: "#10b981", borderRadius: 6 },
                    { type: "bar", label: "Refunds", data: d.refunds_series, backgroundColor: "#ef4444", borderRadius: 6 },
                    { type: "line", label: "Refund rate %", data: d.rate_series, borderColor: "#f59e0b",
...
registry.category("actions").add("mn_finance_insights.refund_rate", RefundRate);
