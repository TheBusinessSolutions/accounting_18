...
export class TaxReconciliation extends Component {
    static template = "mn_finance_insights.TaxReconciliation";
    static props = { "*": true };
    setup() {
    ...
    async load() {
        ...
        try { this.state.data = await this.orm.call("finance.tax.reconciliation", "get_tax_rec_data", [this.state.options]); }
...
registry.category("actions").add("mn_finance_insights.tax_reconciliation", TaxReconciliation);
