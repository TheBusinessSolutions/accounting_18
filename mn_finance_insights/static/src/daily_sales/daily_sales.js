...
export class DailySales extends Component {
    static template = "mn_finance_insights.DailySales";
    static props = { "*": true };
    setup() {
    ...
    async load() {
        ...
        try { this.state.data = await this.orm.call("finance.daily.sales", "get_snapshot_data", [this.state.options]); }
    ...
    renderChart() {
        ...
        const c = this.rootRef.el.querySelector("canvas[data-chart='daily']");
        if (!c) return;
        const d = this.state.data;
        this.chart = new window.Chart(c, {
            type: "bar",
            data: { labels: d.labels.map((l) => l.slice(5)),
                datasets: [{ label: "Daily revenue", data: d.values, backgroundColor: "#10b981", borderRadius: 4 }] },
            options: { responsive: true, maintainAspectRatio: false,
                plugins: { legend: { display: false } },
                scales: { x: { grid: { display: false }, ticks: { maxTicksLimit: 15 } }, y: { grid: { color: "rgba(0,0,0,0.05)" } } } },
        });
    }
}
registry.category("actions").add("mn_finance_insights.daily_sales", DailySales);
